      ...
      <div className="grid grid-cols-1 gap-4 max-w-lg mx-auto">
        {videos.length > 0 ? (
          videos.map((video) => (
            <div
              key={video._id}
              className="p-4 bg-gray-800 rounded-lg shadow-md hover:shadow-lg transition-all duration-200 transform hover:scale-105"
            >
              <img
                src={video.thumbnail}
                alt={video.title}
              ...
              />
              <h2 className="text-md font-semibold mt-2">{video.title}</h2>
              <p className="text-xs text-gray-400">Views: {video.views}</p>
              <p className="text-xs text-gray-400">Likes: {video.likes}</p>
            </div>
          ))
        ) : (
